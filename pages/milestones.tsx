...
interface Milestone { id: string; startup_id: string; title: string; description?: string; target_date?: string; completed: boolean; created_at: string }
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-main, #f5f0eb)' }}>
      ...
      <main style={{ flex: 1, padding: '2.5rem 3rem' }}>
        ...
        {milestones.length > 0 && (
          <div style={{ margin: '1.5rem 0', background: 'var(--bg-card)', borderRadius: '12px', padding: '1.25rem', border: '1px solid var(--border-light)' }}>
            ...
            <div style={{ background: 'var(--border-light)', borderRadius: '99px', height: 8 }}>
              <div style={{ background: 'var(--terracotta)', width: pct + '%', height: '100%', borderRadius: '99px', transition: 'width 0.4s ease' }} />
            </div>
          </div>
        )}
                    ...
                    <div key={m.id} style={{ position: 'relative', marginBottom: '1rem', background: 'var(--bg-card)', borderRadius: '10px', padding: '1rem 1.25rem', border: '1px solid var(--border-light)' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div>
                          ...
                          {m.target_date && <div style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginTop: '0.4rem' }}>🎯 {m.target_date}</div>}
                        </div>
                        <div style={{ display: 'flex', gap: '0.5rem', flexShrink: 0, marginLeft: '1rem' }}>
                          <button className="action-btn action-btn-sage" onClick={() => toggleComplete(m)}>✓ Done</button>
                          <button className="action-btn" style={{ color: '#dc3545' }} onClick={() => deleteMilestone(m.id)}>✕</button>
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        <Modal isOpen={showAdd} onClose={() => setShowAdd(false)} title="Add Milestone">
          <div className="form-group"><label>Title *</label><input className="form-input" value={form.title} onChange={e => setForm(f => ({...f, title: e.target.value}))} placeholder="What's the milestone?" /></div>
