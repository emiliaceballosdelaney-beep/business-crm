...
function NoteModal({ projectId, onClose }: { projectId: string; onClose: () => void }) {
...
export default function ProjectActivityTab({ projectId, notes, addOpen, onAddClose }: Props) {
  return (
    <div>
      {addOpen && <NoteModal projectId={projectId} onClose={onAddClose} />}
      ...
      {notes.length === 0 ? (
        <div className="border border-dashed border-[#E8E0DC] rounded-lg p-8 text-center">
          ...
          <p className="text-[13px] italic text-[#574141]/50 font-body">No activity yet. Use "Log Note" to track progress on this project.</p>
        </div>
      ) : (
