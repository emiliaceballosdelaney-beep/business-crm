        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 20 }}>
            <textarea
              ...
              placeholder="What's happening with this task?"
              disabled={loading}
              autoFocus
            />
          </div>
...
export default function TaskActivityTab({ taskId, notes, addOpen, onAddClose }: Props) {
  return (
    <div>
      {addOpen && <NoteModal taskId={taskId} onClose={onAddClose} />}
      ...
      {notes.length === 0 ? (
        <div className="border border-dashed border-[#E8E0DC] rounded-lg p-8 text-center">
          ...
          <p className="text-[13px] italic text-[#574141]/50 font-body">No activity yet. Use "Log Note" to track progress on this task.</p>
        </div>
      ) : (
